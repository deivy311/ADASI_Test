...
GstElement *pipeline_manager::setup_gst_pipeline(CairoOverlayState *overlay_state, std::string source_type, std::string host, int port, std::string RTSP_port, std::string RTSP_file_path)
{
  ...
  GstElement *pipeline, *src, *overlay, *capsfilter, *videoconvert, *filesink;
  GstElement *tee;
  ...
  gst_bin_add_many(GST_BIN(pipeline), src, adaptor1, overlay, videoconvert, videoscale, capsfilter, encoder, muxer, tee, video_queue, file_queue, filesink, NULL);
